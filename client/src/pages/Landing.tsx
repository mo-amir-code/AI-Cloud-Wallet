...
import { useUserInfo } from "../react-query/hooks";
import type { UserInfoType } from "../types/zustand/user.types";
import { useNavigate } from "react-router-dom";

const Landing: React.FC = () => {
  const { isUserLoggedIn, login, setSettings } = useUserStore();
  const { refetch: fetchUserInfo, isFetching } = useUserInfo();
  ...
  useEffect(() => {
    const handleMessage = async (event: MessageEvent) => {
      ...
      // Only process auth-related messages
  ...
  }, [fetchUserInfo, login, setSettings]);
  ...
  const checkIsuserLoggedIn = async () => {
    try {
      await httpAxios.get(ROUTES.AUTH.ROOT);
      await configureUserInfo();
      return true;
    } catch (error) {
  ...
  const configureUserInfo = async () => {
    // Fetch user info after successful authentication
    const result = await fetchUserInfo();

    if (result.data) {
      const { user, settings } = result.data.data;
      // console.log("User data:", user);
      // console.log("User settings:", settings);

      const usr: UserInfoType = {
        id: user.id,
        email: user.email,
        name: user.name,
      ...
      // Set user info in Zustand store and mark as logged in
      login(usr);
